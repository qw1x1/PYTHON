...
# Пустой except отлавоивает любой тип ошибки
# При этом если указать базовый класс этих двух классов, то он отловит их ошибки, но не дас инфы о ней
# Для этого мы и вызоваем опеделенные классы для определенных ошибок, чтобы понимать какая именно ошибка возникла 
def try_except():
    try:
        lst = [12, 4]
        x, y = map(int, lst)
        res = x / y
        print(res)
    # Если прописать базовый класс перед дочерними, то он отловит все их ошибки и дочерние классы ни как не смогут обработать ошибку
    # except Exception:
    #     print('Отловит все ошибки своих дочкрних классов но не даст точной инфы о ней')

    # Класс ValueError отлавливает ошибки типа данных
    except ValueError:
        print('Ошибка типа данных')

    # Класс ZeroDivisionError отлавливает ошибку при делении на ноль
    except ZeroDivisionError:
        print('Деление на ноль')

    print('Дальнейшее выполнение программы даже если в блоке try / except возникнет исключение')
...
def inally_else():
    ...
    try:
        ...
        print(f'Ошибка типа данных {v}')

    else:
        print('Выполняется только если не возникло ни одного исключения')
        print('Всегда стоит перед блоком finally')

    # При обяклении блоков try / except / finally в функции то блок finally выполниться до return
    finally:
...
def func11():
    # Пусть нижние уровни программы выполняют только свою логику, а обработку исключений лучше выполнять на верхнем уровне программы (__main__)
    def func1():
        return 1 / 0
    ...
    # Обработку исключений лучше делать на верхнем глобальном уровне программы (__main__), 
    # а функции и классы пусть выполняют только свою логику (только свою задачу)
    # Нижние уровни программы должны выполнять 
    try:
        func1()
    except:
        print('Обработана ошибка, верхнего уроыня функции func1')
...
Если вы думаете, нужно ли вам их использовать — 
вам не нужно (люди, которым они реально нужны, точно знают, зачем они им, и не нуждаются в объяснениях, почему).
'''
